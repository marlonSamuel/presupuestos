...
model.presupuestoMController = {
    ...
    loteId: ko.observable(""),
    fincaId: ko.observable(""),
    loteProductor: ko.observable(""),
    titleDescripcionContable: ko.observable(""),
    ...
    mapDetalleContable: function (contable) {
        ...
        eContable.detalle(contable.detalle);
    },
    ...
    saveFile: function () {
        ...
        for (var i = 0; i < arrayMasivo.length; i++) {
            model.presupuestoMController.clearDetalleCarga();
            model.presupuestoMController.selectSubcentroByCarga(arrayMasivo[i].subcentro_costo);

            var lote = arrayMasivo[i].lote;
            if (lote != undefined) {
                model.presupuestoMController.selectSubloteByCarga(arrayMasivo[i].lote);
                model.presupuestoMController.loteProductor(" codigo lote");
            } else {
                model.presupuestoMController.selectSubloteByCarga(arrayMasivo[i].productor);
                model.presupuestoMController.loteProductor(" codigo productor");
            }
            ...
            if (codigoTipo === 'P_CT') {
                nDetalle.subcentroId = model.presupuestoMController.dataByCarga.fincaId();
                nDetalle.loteId = model.presupuestoMController.dataByCarga.loteId();
                ...
                nDetalle.detalle = arrayMasivo[i].detalle;
            } else if (codigoTipo === 'P_CS') {
                nDetalle.fincaId = model.presupuestoMController.dataByCarga.fincaId();
                nDetalle.subloteId = model.presupuestoMController.dataByCarga.loteId();
        ...
        model.presupuestoMController.validateCargaPresupuesto(cargaParams.detalle);


        //validamos que los datos sean correctos
        if (!model.presupuestoMController.validateCarga.isValid()) {
            bootbox.alert(model.presupuestoMController.validateCarga.message());
            $.unblockUI();
            model.presupuestoMController.clearInput();
            model.presupuestoMController.clearDetalleCarga();
            return;
    ...
    selectSubcentroByCarga: function (codigo) {
        var subccostos = model.presupuestoMController.subccostos();
        for (var i = 0; i < subccostos.length; i++) {
            if (codigo === subccostos[i].codigo || codigo === subccostos[i].codigo_nav) {
                model.presupuestoMController.dataByCarga.fincaId(subccostos[i].Id);
            }
        }
    },
    ...
    selectSubloteByCarga: function (codigo) {
        var lotes = model.presupuestoMController.lotes();
        for (var i = 0; i < lotes.length; i++) {
            if (codigo === lotes[i].codigo) {
                model.presupuestoMController.dataByCarga.loteId(lotes[i].Id);
            }
        }
    },
    ...
    clearDetalleCarga: function () {
        model.presupuestoMController.dataByCarga.fincaId("");
        model.presupuestoMController.dataByCarga.loteId("");
    },

    viewDetalleCuentaContable: function (cuenta) {
        model.presupuestoMController.mapDetalleContable(cuenta);
        model.presupuestoMController.titleDescripcionContable(cuenta.codigo_cuenta + '-' + cuenta.cuenta_contable);
    },

    //validamos la data contable
    validateCargaPresupuesto: function (data) {
        var isValidate = false;
        model.presupuestoMController.validateCarga.isValid(true);
        var codigo = model.presupuestoMController.codigoPresupuesto();
        for (var i = 0; i < data.length; i++) {

            if (codigo === 'P_CT') {

                if (data[i].subcentroId === undefined || data[i].subcentroId === "") {
                    model.presupuestoMController.validateCarga.isValid(false);
                    model.presupuestoMController.validateCarga.message(
                        "Error en la linea numero " + parseInt(i + 1) + " codigo subcentro de costo no existe"
                    );
                ...
                if (!isValidate) {
                    if (data[i].loteId === undefined || data[i].loteId === "") {
                        model.presupuestoMController.validateCarga.isValid(false);
                        model.presupuestoMController.validateCarga.message(
                            "Error en la linea numero " + parseInt(i + 1) + model.presupuestoMController.loteProductor()+"  no existe"
                        );
                        isValidate = true;
                    }
                }
                ...
                if (data[i].fincaId === undefined || data[i].fincaId === "") {
                    model.presupuestoMController.validateCarga.isValid(false);
                    model.presupuestoMController.validateCarga.message(
                        "Error en la linea numero " + parseInt(i + 1) + " codigo subcentro de costo no existe"
                    );
                ...
                if (!isValidate) {
                    if (data[i].subloteId === undefined || data[i].subloteId === "") {
                        model.presupuestoMController.validateCarga.isValid(false);
                        model.presupuestoMController.validateCarga.message(
                            "Error en la linea numero " + parseInt(i + 1) + model.presupuestoMController.loteProductor() +" no existe"
                        );
            ...
            if (!isValidate) {
                if (codigo === 'P_CT') {
                    var cuentas = model.presupuestoMController.cuentas();
                    var isValidCuenta = false;
                    for (var j = 0; j < cuentas.length; j++) {
                        if (data[i].codigo_cuenta === cuentas[j].numero) {
                            isValidCuenta = true;
                        }
                    }
                    ...
                    if (!isValidCuenta) {
                        model.presupuestoMController.validateCarga.isValid(false);
                        model.presupuestoMController.validateCarga.message(
                            "Error en la linea numero " + parseInt(i + 1) + " codigo de cuenta " + data[i].codigo_cuenta + " no existe"
                        );
                    }
                } else if (codigo === 'P_MO') {
                    var actividades = model.presupuestoMController.actividades();
                    var isvalidActividad = false;
                    for (var j = 0; j < actividades.length; j++) {
                        if (data[i].referencia === actividades[j].codigo) {
                    ...
                    if (!isvalidActividad) {
                        model.presupuestoMController.validateCarga.isValid(false);
                        model.presupuestoMController.validateCarga.message(
                            "Error en la linea numero " + parseInt(i + 1) + " codigo actividad " + data[i].referencia + " no existe"
                        );
                    }
                } else if(codigo === 'P_AG' || codigo === 'P_VT'){
                    var productos = model.presupuestoMController.productos();
                    var isValidProducto = false;
                    for (var j = 0; j < productos.length; j++) {
                        if (data[i].referencia === productos[j].codigo) {
                            isValidProducto = true;
                        }
                    }
                    ...
                    if (!isValidProducto) {
                        model.presupuestoMController.validateCarga.isValid(false);
                        model.presupuestoMController.validateCarga.message(
                            "Error en la linea numero " + parseInt(i + 1) + " codigo producto " + data[i].referencia + " no existe"
                        );
                    }
                }
            }
